...
function AIAssistants() {
  ...
  const insertAssistant= useMutation(api.userAiAssistants.InsertSlectedAssistants);
    ...
    <div className='px-10 mt-20 md:px-28 lg:px-36 xl:px-48'>
      ...
      <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5 mt-5'>
        {AiAssistantsList.map((assistant, index)=>(
          <BlurFade key={assistant.image} delay={0.25 + index * 0.065} inView>
          <div key={index} className='hover:border p-3 rounded-xl hover:scale-105 transition-all ease-in-out cursor-pointer relative' onClick={()=>onSelect(assistant)}>
            <Checkbox className='absolute m-2 z-50' checked={IsAssistantSelected(assistant)}/>
