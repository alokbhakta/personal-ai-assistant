...
function AssistantList() {
  ...
  return (
    <div className='p-5 bg-gray-300 bg-secondary border-r-[1px] h-screen relative'>
      ...
      <div className='mt-5'>
        {assistantList.map((assistant_,index)=>(
          <BlurFade key={assistant_.image} delay={0.25 + index * 0.065} inView>
            <div className={`p-2 flex gap-3 items-center hover:bg-gray-200 hover:dark:bg-slate-700 rounded-xl mt-2 cursor-pointer
            ...
            key={index} onClick={()=>setAssistant(assistant_)}>
                ...
                />
                <div>
                    <h2>{assistant_.name}</h2>
                    <h2 className='text-gray-700 text-sm dark:text-gray-200'>{assistant_.title}</h2>
                </div>
            </div>
            </BlurFade>
        ))}
        ...
        <DropdownMenu>
         <DropdownMenuTrigger asChild>
           <div className='absolute bottom-10 flex gap-3 items-center hover:bg-gray-200 w-[90%] p-2 rounded-xl cursor-pointer'>
             {user && <Image src={user?.picture} alt='user' width={35} height={35} 
