...
function ChatUi() {
  ...
  return (
    <div className='mt-20 p-6 relative h-[88vh]'>
          ...
          {messages.map((msg,index)=>(
            ...
            >
              <div className='flex gap-3'>
                ...
                />}
                <div className={`p-3 rounded-lg flex gap-2 ${msg.role=='user'?"bg-gray-300 text-black rounded-lg"
                  :"bg-gray-100 text-black"}`}>
                    {loading && messages?.length-1==index &&<Loader2Icon className='animate-spin'/>}
                      <h2 dangerouslySetInnerHTML={{ __html: msg.content }} />
                </div>
              </div>
            </div>
          ))}
      ...
      <div className='flex justify-between p-5 gap-5 absolute bottom-5 w-[95%]'>
